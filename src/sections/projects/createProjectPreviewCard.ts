...
const createProjectPreviewCard = (world: World, config: ResponsiveConfig) => {
    ...
    closeButton.setAttribute('aria-label', 'Projektvorschau schliessen');
    ...
    const renderProject = (project: PortfolioProject): void => {
        ...
        card.setAttribute(
            'aria-label',
            `Projektvorschau ${project.title}. Enter drücken für Details. Mit den Pfeiltasten durch die Projekte blättern.`,
        );
        card.style.setProperty(
            '--project-preview-accent-rgb',
            getConstellationColorRgb(project.constellation.id),
        );
        eyebrow.textContent = `Konstellation ${label}`;
