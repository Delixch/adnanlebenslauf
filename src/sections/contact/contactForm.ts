...
const validateContactField = (field: ContactField, value: string): string | undefined => {
    ...
    if (field === 'name') {
        ...
        return length > 80 ? 'Höchstens 80 Zeichen verwenden.' : undefined;
    }
    ...
    if (field === 'email') {
        ...
        if (length > 254) {
            return 'Höchstens 254 Zeichen verwenden.';
        }
        return EMAIL_PATTERN.test(value.trim()) ? undefined : 'Gültige E-Mail-Adresse eingeben.';
    }

    if (length < 10) {
        return 'Mindestens 10 Zeichen eingeben.';
    }
...
const contactFormMessages: Record<ContactFormStatus, string> = {
    ...
    'rate-limited': 'Bitte einen Moment warten, bevor Sie eine weitere Nachricht senden.',
    'network-failure': 'Der Kontaktdienst ist momentan nicht verfügbar. Bitte später erneut versuchen.',
    'server-failure': 'Der Kontaktdienst ist momentan nicht verfügbar. Bitte später erneut versuchen.',
};
